...
export const useFlashAnimation = (arbitrageData: Record<string, ArbitrageData>) => {
    ...
    Object.entries(arbitrageData).forEach(([pair, data]) => {
      ...
      if (prevData && isNewData) {
        ...
        const hasChanged = (prev: any, current: any, fieldName: string) => {
          ...
          if (changed) {
            console.log(`📈 ${fieldName} changed: ${prevNum} -> ${currentNum} (diff: ${diff.toFixed(6)})`);
          }
          return changed;
        };
        ...
        // 買い価格の変化をチェック
        const buyPriceChanged = hasChanged(prevData.buy_price, data.buy_price, 'buy_price');
        if (buyPriceChanged) {
        ...
        // 売り価格の変化をチェック
        const sellPriceChanged = hasChanged(prevData.sell_price, data.sell_price, 'sell_price');
        if (sellPriceChanged) {
        ...
        const amountChanged = hasChanged(prevData.amount, data.amount, 'amount');
        if (amountChanged) {
